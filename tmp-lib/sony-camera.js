...
(function () {
    ...
    SonyCamera.prototype.startViewfinder = function (req, res) {
        ...
        this.call('startLiveview', null, function (err, output) {
            if (!output) return new Error("No liveview url!");
    ...
    SonyCamera.prototype.capture = function (callback) {
        var self = this;
        ...
        var processCaptureResult = function(err, output) {
            ...
            if (err) {
                if(err.length > 0 && err[0] == 40403) { // capture still in progress
                    return self.call('awaitTakePicture', null, processCaptureResult);
                } else
            ...
            //     callback && callback(null, {photoName, url, buffer: Buffer.concat(rawData)});
            //   });
            // }).on('error', function(e) {
            //   callback && callback(e);
            // });
    ...
    SonyCamera.prototype.zoomIn = async function (callback) {
        await this.call('actZoom', ['in', 'start'], callback);
        this.call('actZoom', ['in', 'stop'], callback);
    };

    SonyCamera.prototype.zoomOut = async function (callback) {
        await this.call('actZoom', ['out', 'start'], callback);
        this.call('actZoom', ['out', 'stop'], callback);
    };
